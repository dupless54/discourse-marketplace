...
describe Marketplace::Notifier do
  ...
  describe ".notify_transaction_created" do
    it "notifies the seller only, with the buyer as the acting username" do
      ...
      expect(data["transaction_id"]).to eq(transaction.id)
    ...
    it "keeps notifications for two buyers on one listing distinguishable" do
      other_buyer = Fabricate(:user)
      first = build_transaction
      second = build_transaction(buyer: other_buyer)

      described_class.notify_transaction_created(first.id)
