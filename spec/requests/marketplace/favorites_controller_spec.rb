...
describe Marketplace::FavoritesController do
  ...
  describe "#create" do
    ...
    it "favorites a visible listing for the current user" do
      ...
      expect {
        post "/marketplace/listings/#{listing.id}/favorite.json"
      }.to change { Marketplace::Favorite.where(user_id: user.id, listing_id: listing.id).count }.by(1)
  ...
  describe "#destroy" do
    it "removes only the current user's favorite and is idempotent" do
      ...
      expect(Marketplace::Favorite.exists?(user_id: other_user.id, listing_id: listing.id)).to eq(true)
    end
  ...
  describe "#index" do
    ...
    it "returns only the current user's visible favorites newest first" do
      ...
      own_old = Fabricate(:marketplace_favorite, user: user, listing: older, created_at: 2.hours.ago)
      own_new = Fabricate(:marketplace_favorite, user: user, listing: listing, created_at: 1.hour.ago)
      ...
      expect(body["listings"].map { |item| item["id"] }).to eq([own_new.listing_id, own_old.listing_id])
