...
module(
  "Integration | Component | MarketplaceTransactionCenter",
  function (hooks) {
    ...
    test("shows Confirm and Cancel for a pending transaction awaiting the viewer's own confirmation", async function (assert) {
      ...
      assert
        .dom(".marketplace-transaction-center__actions .btn-primary")
        .exists("Confirm is shown");
      assert
        .dom(".marketplace-transaction-center__actions .btn")
        .exists("Cancel is shown");
    });
    ...
    test("links each card to the exact listing/transaction via a transaction_id query param", async function (assert) {
      ...
      assert.true(
        href.includes("/marketplace/listings/10"),
        "links to the exact listing"
      );
      assert.true(
        href.includes("transaction_id=1"),
        "carries the exact transaction id"
      );
    });
